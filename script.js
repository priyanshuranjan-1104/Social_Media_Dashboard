...
if (canvas) {
  ...
  function drawChart(W, H) {
    ...
    const likes = Array.from({ length: DAYS }, (_, i) =>
      Math.max(0.1, waveVal(i, 0) * 3200 + 800),
    );
    const comments = Array.from({ length: DAYS }, (_, i) =>
      Math.max(0.1, waveVal(i, 2) * 900 + 200),
    );
    const shares = Array.from({ length: DAYS }, (_, i) =>
      Math.max(0.1, waveVal(i, 5) * 600 + 100),
    );
    ...
    labelIdxs.forEach((idx, li) => ctx.fillText(labels[li], toX(idx), H - 6));
  }
